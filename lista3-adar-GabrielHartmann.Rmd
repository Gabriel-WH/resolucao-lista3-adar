...
Não consegui gerar o html, por diversas vezes rodei o knitr por mais de 30 min e o computador travou depois. Mas todos os chunks rodaram individualmente.
...
```{r}
q1_url <- "http://www.aos.wisc.edu/~dvimont/MModes/RealTime/AMM.txt"
...
```{r}
q2_url <- "https://iri.columbia.edu/~forecast/ensofcst/Data/archive/ensofcst_cpc_ALL"
q2_txt <- read_lines(q2_url)
...
**(i)**
R: Cada coluna é uma variável meteorológica em uma hora do dia, ou seja, cada variável tem 24 colunas (menos a Radiação Global, que só possui dados de 9h até 22h). A coluna inicial, que não diz respeito a uma hora, é a data da observação. Cada linha é um dia de observações.

**(ii)**
R: São 182 colunas de variáveis, então:
```{r}
((182 - (22 - 9 + 1)) / 24) + 1
```
...
**(iii)**
R: Não, a Radiação Global só possui observações das 9h até as 22h, diferente das demais variáveis, que possuem observações das 00h até as 23h.
...
q5_nomes <- c("data", "PRESSÃO ATMOSFERICA hPa","VENTO VELOCIDADE", "VENTO DIREÇÃO graus", "RADIACAO GLOBAL KJM",
...
```{r}
nomes_novo <- c("data", "p_hpa","vel_ms", "dir_graus", "radglob_kjm2",
...
library(tictoc)
tic()
wtd_df <- raster::as.data.frame(wtd, xy = TRUE, na.rm = TRUE)
toc()
str(wtd_df)
...
Porque a função `as.data.frame` do pacote `raster` com o argumento `na.rm` como `TRUE` faz com que as linhas sem valores (valor = `NA`) sejam removidas. Assim, enquanto o número de células (observações) do objeto importado é 62366400, o número de linhas do *data frame* é 22347186.
...
```{r,eval = FALSE}
q7_url <- "http://www.bom.gov.au/climate/mjo/graphics/rmm.74toRealtime.txt"
...
```{r, eval = FALSE}
q7_arq <- "rmm.74toRealtime.txt"

download.file(url = q7_url, destfile = q7_arq)
```

Como houve um problema em acessar o site diretamente através da função `import` (`cannot open URL 'http://www.bom.gov.au/climate/mjo/graphics/rmm.74toRealtime.txt': HTTP status was '403 Forbidden'`), e também através da função `download.file` - devido ao mesmo problema -, farei o download pelo browser, que, por algum motivo, abre sem problema algum.

```{r}
q7_arq <- "download_browser//rmm.74toRealtime.txt"
...
q7_data <- import(
                  ...
                  format = "tsv",
                  header = FALSE,
                  na.strings = c("999","9.99999962E+35"),
                  col.names = c(scan(
                                   file = q7_arq,
                                   #sep = ",",
                                   skip = 1,
...
names(q7_data) <- gsub(pattern = "[.]", replacement = "", x = names(q7_data))
names(q7_data) <- gsub(pattern = "[,]", replacement = "", x = names(q7_data))
...
O link passado não existe mais, mas acredito que achei a planilha referida neste outro link: https://portal.inmet.gov.br/uploads/normais/Temperatura-Minima_NCB_1961-1990.xls
...
Download feito através do browser.

   
   a. Importe os dados para o R, converta-os para *data frame* e verifique o número de colunas e linhas resultantes. 

```{r}
q10_arq <- "prec_daily_UT_Brazil_v2.2_20100101_20151231.nc"

#download.file(url = q10_url,
#              destfile = q10_arq,
#              mode = "wb"
#)
```
